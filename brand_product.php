...
if (isset($_GET['id'])) {
    ...
    $select_brand_name = $conn->prepare("SELECT name FROM brands WHERE id = ?");
    ...
    if ($result_brand_name->num_rows > 0) {
        $brand_name = $result_brand_name->fetch_assoc()['name'];
    }
}
?>
...
<body>
    ...
    <div class="banner">
        <div class="detail">
            <h1 style="padding: 80px;">Thương hiệu <?php echo $brand_name; ?></h1>
        </div>
    </div>
    ...
    <div class="box-container">
        ...
        if (isset($_GET['id'])) {
            ...
            $select_brands = $conn->prepare("SELECT name FROM brands WHERE id = ?");
            $select_brands->bind_param("i", $brand_id);
            $select_brands->execute();
            $result_brands = $select_brands->get_result();

            if ($result_brands->num_rows > 0) {
                $brand_name = $result_brands->fetch_assoc()['name'];

                // Lấy sản phẩm liên quan đến tên thương hiệu đã chọn
                $status = 'Đang hoạt động';
                $select_products = $conn->prepare("SELECT * FROM `products` WHERE BINARY brands = ? AND status = ?");
