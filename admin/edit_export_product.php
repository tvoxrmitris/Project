...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_product'])) {
    ...
    // Lấy `subcategory_name` từ `subcategory_id`
    $query_subcategory_name = "SELECT subcategory_name FROM subcategory WHERE subcategory_id = ?";
    $stmt_subcategory_name = $conn->prepare($query_subcategory_name);
    $stmt_subcategory_name->bind_param("i", $subcategory_id);
    $stmt_subcategory_name->execute();
    $stmt_subcategory_name->bind_result($subcategory_name);
    $stmt_subcategory_name->fetch();
    $stmt_subcategory_name->close();

    if (!$subcategory_name) {
        echo "Không tìm thấy danh mục phụ.";
        exit;
    }

    // Thực hiện cập nhật trong cơ sở dữ liệu
    $update_query = "UPDATE detail_import_product
                     SET product_name = ?, 
                         quantity_stock = ?, 
                         import_price = ?, 
                         code_color = ?, 
    ...
    $update_stmt->bind_param("siissssi", $name, $quantity, $price, $code_color, $capacity, $category, $subcategory_name, $detail_import_id);
    ...
    <div id="content-wrapper">
        ...
        <section class="add-products form-container">
            <form method="POST" action="" enctype="multipart/form-data">
                <a style="font-size: 25px;" href="detail_export.php" class="back-arrow">&#8592;</a>
                <?php
                ...
                if ($detail_import_id > 0) {
                    ...
                    $query = "SELECT product_name, import_price, quantity_stock, code_color, capacity, category_name, subcategory_name 
                      FROM detail_import_product
                      WHERE detail_import_id = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("i", $detail_import_id);
                    $stmt->execute();
                    $stmt->bind_result($product_name, $import_price, $quantity_stock, $code_color, $capacity, $category_name, $subcategory_name);
                    $stmt->fetch();
                    $stmt->close();
                }
