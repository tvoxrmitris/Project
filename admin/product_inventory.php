...
<body>
    ...
    <section class="shop">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                                    $inventory_id = $fetch_products['inventory_id'];
                                    $check_request_query = "SELECT * FROM low_quantity_stock WHERE inventory_id = '$inventory_id'";
                                    $check_request_result = mysqli_query($conn, $check_request_query);

                                    if (mysqli_num_rows($check_request_result) > 0) {
                                        // Nếu đã có yêu cầu, hiển thị "Hủy yêu cầu"
                                        echo '<a id="requestLink_' . $inventory_id . '" style="' . $lowStockStyle . ' font-weight: bold; cursor: pointer;" onclick="toggleRequest(' . $inventory_id . ', \'delete\'); return false;">Hủy yêu cầu</a>';
                                    } else {
                                        // Nếu chưa có yêu cầu, hiển thị "Yêu cầu nhập hàng"
                                        echo '<a id="requestLink_' . $inventory_id . '" style="' . $lowStockStyle . ' font-weight: bold; cursor: pointer;" onclick="toggleRequest(' . $inventory_id . ', \'request\'); return false;">Yêu cầu nhập hàng</a>';
                                    }
                                    ?>
                            <!-- Thêm các nút hành động ở đây -->
                            <a style="<?php echo $lowStockStyle; ?>"
                                href="edit_product_inventory.php?inventory_id=<?php echo $fetch_products['inventory_id']; ?>"
                                class="edit-button">Sửa</a>
                            <a style="<?php echo $lowStockStyle; ?>"
                                href="delete_product.php?inventory_id=<?php echo $fetch_products['inventory_id']; ?>"
                                class="delete-button"
        ...
        function toggleRequest(inventoryId, action) {
            ...
            fetch('request_quantity_stock.php', {
                    method: 'POST',
                    headers: {
