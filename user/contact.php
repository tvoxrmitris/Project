<?php
    ...
    if(isset($_POST['submit-btn'])){
        ...
        $select_message = mysqli_query($conn, "SELECT * FROM `message` WHERE user_name='$name' AND user_email='$email' AND number='$number' AND message='$message'") or die('query failed');
        if(mysqli_num_rows($select_message)>0){
            echo 'Tin nhắn đã được gửi';
        }else{
            mysqli_query($conn, "INSERT INTO `message`(`user_id`, `user_name`, `user_email`, `number`, `message`) VALUES ('$user_id', '$name', '$email', '$number', '$message')") or die('query failed');
        }
    }
