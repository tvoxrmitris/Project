...
if(isset($_POST['add_to_cart'])){
    ...
    $cart_query = mysqli_query($conn, "SELECT * FROM `cart` WHERE product_name='$product_name' AND user_id ='0'") or die('query failed');
    ...
    if(mysqli_num_rows($cart_query) > 0){
        ...
        if($new_quantity <= $stock_available) {
            ...
            mysqli_query($conn, "UPDATE `cart` SET `$current_qty_column`='$new_quantity', `$current_price_column`='$new_price' WHERE product_name='$product_name'");
        ...
        if($stock_available > 0) {
            ...
            mysqli_query($conn, "INSERT INTO `cart`(`pid`, `product_name`, `product_image`, `qtyfor$selectedCapacity`, `pricefor$selectedCapacity`) VALUES('$product_id', '$product_name', '$product_image', '1', '$product_price')");
...
<div class="about-us">
    <div class="row">
        ...
        <div class="img-about" style="position: relative;"> <!-- Đặt position là relative -->
...
<?php
$select_products = mysqli_query($conn, "SELECT * FROM products WHERE status = 'Còn hàng'") or die('Query failed');
